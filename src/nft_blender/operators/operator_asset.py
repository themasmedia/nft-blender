...
from nft_blender.nft_bpy import nft_anim
from nft_blender.nft_bpy import nft_io
from nft_blender.nft_bpy import nft_mtl
...
def anim_import_keying_set():
    ...
    #
    collection_name = nft_ui.ui_get_item(
    ...
    keying_set_names = nft_anim.anim_get_keying_set_names_for_asset(
        asset_name=collection_name,
        exists_ok=True,
    )
    keying_set_name = nft_ui.ui_get_item(
    ...
    if keying_set_name:
        ...
        nft_anim.anim_load_keying_set_for_asset(
            asset_name=collection_name,
        ...
        if nft_ui.ui_message_box(
            ...
            frame_number = nft_ui.ui_get_int(
                title='Set Keyframe',
...
def script_update_asset():
    """TODO"""
    os.system('cls')

    collection_data = list(COLLECTION_DATA.items())
    collection_name = None
    operation = nft_ui.ui_get_item(
    ...
    if operation == 'Append':
        ...
        asset_file_path = nft_ui.ui_get_file(
            caption='Select File',
            dir='{assets_dir}}',
    ...
    nft_io.io_append_file(blend_file_path, inner_path, collection_name)
...
def script_set_material_data():
    """
nft_scripts.script_set_material_data()
    """
    mesh_objs = nft_scn.scn_get_selected_objects(['MESH'])
    if not mesh_objs:
        mesh_objs = (obj for obj in bpy.data.objects if obj.type == 'MESH')

    material_name = nft_ui.ui_get_item(
        title='Select Material',
        label='',
        items=[material.name for material in bpy.data.materials if not material.is_grease_pencil],
    )
    nft_mtl.mtl_set_material_data(material_name, mesh_objs)
